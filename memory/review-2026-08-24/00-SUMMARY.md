...
> **2026-08-25 冒烟更新**：Step 1-9 已全部完成（服务层直驱方式），验收报告见 `SMOKE-REPORT-2026-08-25.md`。
> - P0-4 已修复并实测通过：commit `f4ae99c9`（ResolveAppeal 改用真实 element_id + JWT issuer 统一）
> - 此前 4 个冒烟阻断 bug 已修：commit `35e85689`
> - 剩余待办：P0-1/2/3/5/6 仍未修（见上表）；HTTP 层 404 异常谜题未解（详见报告"未解问题"节）

1. 按 P0-1~6 顺序修复剩余 5 项（P0-4 ✅ 已完成）
2. HTTP 层带 X-Tenant-ID 头请求返回路由级 404 的异常需专项排查（服务层直驱不受影响，疑似请求/响应交叉污染或环境干扰）
3. 修复后回归 go test + tsc + 冒烟
